...
void PS()
{
    ...
    // Get fog factor
        ...
        #else
            vec3 env = cMatEnvMapColor * textureCube(sZoneCubeMap, reflect(vZoneReflectionVec, normal)).rgb;
            finalColor = diff * lightColor * env.rgb * diffColor.rgb;
        #endif
        
        vec3 env = cMatEnvMapColor * textureCube(sZoneCubeMap, reflect(vZoneReflectionVec, normal)).rgb;
        finalColor += (env.rgb * diff);
